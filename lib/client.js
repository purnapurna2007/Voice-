...
try {
    ...
    console.log("📡 Connected to the RCD DB")
} catch {
...
let cc = Config.sessionName.replace(/FLASH-MD-WA-BOT;;;=>/g,""); // Dont Remove "SITHUWA-MD;;;" So that You Can Use Secktor Qr too in your bot
async function MakeSession(){
if (!fs.existsSync(__dirname + '/auth/creds.json"')) {
    if(cc.length<30){
        let { data } = await axios.get('https://paste.c-net.org/'+cc)
        await fs.writeFileSync(__dirname + '/auth/creds.json"', && session != "zokk") {    
    } else {  await fs.writeFileSync(__dirname + '/auth/creds.json"', atob(session), "utf8")  }
}
}
MakeSession()
setTimeout(() => {
